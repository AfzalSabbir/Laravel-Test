    ...
    if(Request::hasFile($image)){
      if (is_array($name)) {
        ...
        */
        ...
        $location = ($target_location.'/'.$filename);
        Image::make($file)->resize(48, 48)->save($location);

        $filename = $name[0]. '.'.$file->getClientOriginalExtension();
        $location = ($target_location.'/'.$filename);
        Image::make($file)->resize($width, $height)->save($location);
  ...
  public static function update($image, $file, $name, $target_location, $old_location, $width=500, $height=500)
  {
    if(Request::hasFile($image)){
      if (is_array($name)) {
        ...
        File::delete(explode('.', $old_location)[0].'_short.'.explode('.', $old_location)[1]);
