...
@section('fav_title', __('backend/category.category') )
...
<div class="app-title">
  <div>
    <h1><i class="{{ 'fa fa-500px' }}"></i> {{ __('backend/category.category') }} {{ __('backend/default.management') }}</h1>
  </div>
  <ul class="app-breadcrumb breadcrumb">
    <li class="breadcrumb-item"><i class="fa fa-home fa-lg"></i><a href="{{ route('admin.home') }}">{{ __('backend/default.dashboard') }}</a></li>
    <li class="breadcrumb-item">{{ __('backend/category.category') }}</li>
  </ul>
</div>
  ...
  <div class="col-md-12">
    <div class="card">
      ...
      <div class="card-header">
        <div class="row">
          <div class="col-md-6"><h2><i class="{{ 'fa fa-500px' }}"></i> {{ __('backend/category.category_list') }}</h2></div>
      ...
      <div class="card-body" data-my-table="table-1">
        ...
        <div class="toggle-table-column">
          ...
          {{-- 
           ...
           --}}
          <a href="#" class="toggle-vis hide-on-load" data-column="1"><b>{{ __('backend/default.status') }}</b></a> |
        ...
        <div class="table-responsive pt-1">
          ...
          --}}
          <table {{-- id="my-table" --}} id="datatable" class="table table-bordered table-hover display">
            ...
            <tbody>
              ...
              <tr class="{{ $row->status == 0 ? 'inactive_':'' }}">
                ...
                <td class="action">
                  <div class="btn-group">
                    @foreach($permissions->submenus as $key => $permission)
                      @if(\App\Models\Menu::checkBodyMenu($permission->id, $myRole->in_body))
                        @if($key == 0)
                          <button type="button" title="View" data-toggle="modal" data-target="#this_view" class="btn btn-info text-white"><i class="fa fa-eye"></i></button>
                        @elseif($key == 1)
                          <button type="button" title="Edit" data-toggle="modal" data-target="#edit_page-{{ $md5_id }}" class="btn btn-warning text-white"><i class="fa fa-edit"></i></button>
                        @else
                  ...
                  <!-- Edit Modal -->
                  <div class="modal fade" id="edit_page-{{ $md5_id }}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="editModalLabel"><i class="fa fa-pencil-square"></i> {{ __('backend/category.category') }} {{ __('backend/default.update') }}</h5>
                          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                          </button>
                        </div>
                        <div class="modal-body">
                          <form action="{{ route('admin.category.update', encrypt($row->id)) }}" method="POST">
                            @csrf
...
<div class="modal fade" id="add_new" tabindex="-1" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="addModalLabel"><i class="fa fa-plus-square"></i> {{ __('backend/category.add_category') }}</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<div class="modal fade" id="this_view" tabindex="-1" role="dialog" aria-labelledby="viewModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="viewModalLabel"><i class="fa fa-eye"></i> {{ __('backend/category.view_category') }}</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
